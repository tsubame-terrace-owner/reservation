...
### 1. トップページからの先読み（`docs/index.html` → `docs/form.html`）
...
- 先読みが無い／古い／壊れている場合は、何もせず通常どおりサーバから取得する

### 2. 集計時に読む列を絞る（`getActiveBookingRows()`）

空き状況の集計に要るのは status / reservationDate / slot / 人数 だけなので、
**C列〜L列の10列のみ**読む（従来は全21列）。予約シートは行が増え続けるので効果は年々効いてくる。

- 使うのは `getAvailabilityRange()` と `getSlotAvailability()` の2箇所だけ
- **メール送信・重複チェックは全列必要**なので従来の `getAllActiveReservations()` のまま
- 列順を変えたら壊れるが、`RESERVATION_HEADERS` から位置を計算し、想定外なら全列読み込みに
  フォールバックする保険を入れてある

⚠️ **この施策の速度改善は実測では確認できなかった**（2026-08-30 時点）。読むセル数は
126→60 に半減したが、**GAS の応答揺らぎが同条件で 1.02〜2.57秒**あり、狙っていた
300〜500ms を完全に飲み込んでしまう。効果は「行数が増えても遅くなりにくい」という
構造的なものにとどまる。

→ **教訓: GAS 応答の 500ms 未満の最適化は、測っても効果を確認できない**。
やるなら「秒単位で消せるか」を基準にすること（＝先読みのように待ち時間ごと隠す方向）。

### 検討したが見送った案

**`CacheService` で空き状況をキャッシュ**：当初60秒TTLで提案したが、この規模（1時間に数件の閲覧）では
**次の人が来る前に期限切れになりヒットしない**。やるなら「予約・変更・キャンセルの4箇所で破棄する前提で
TTLを数時間に伸ばす」形にすること。予約頻度が低いことは、キャッシュにとって不利ではなく有利な条件。
シート直編集に備えて onEdit トリガーでの破棄も要る。
